...
String fechaFin = "" + fecha.get(Calendar.YEAR) + "-" + strMes + "-" + strDia;
    ...
    public ResponseEntity<Resource> reporteVetasTotales(
    ...
    public ResponseEntity<Resource> reporteProductos(@RequestParam String tipo)
     throws IOException {
        ...
        @GetMapping("/productosTotales")
    public ResponseEntity<Resource> reporteProductosTotales(
            @RequestParam String precioMin,
            @RequestParam String PrecioMayor,
            @RequestParam String tipo)
            throws IOException {
       
        Map<String, Object> parametros = new HashMap();
        parametros.put("PrecioMenor", precioMin);
        parametros.put("PrecioMayor", PrecioMayor);
        var reporte = "productosTotales";
        return reporteService.generaReporte(reporte, parametros, tipo);
    }
}
